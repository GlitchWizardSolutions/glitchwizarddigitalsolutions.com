...
$tickets = $pdo->query('SELECT * FROM project_tickets')->fetchAll(PDO::FETCH_ASSOC);
// Check whether the comment ID is specified
if (isset($_GET['id'])) {
    // Retrieve the comment from the database
    $stmt = $pdo->prepare('SELECT * FROM project_tickets_comments WHERE id = ?');
    ...
    if (isset($_POST['submit'])) {
        // Update the comment
        $stmt = $pdo->prepare('UPDATE project_tickets_comments SET msg = ?, ticket_id = ?, acc_id = ?, created = ?  WHERE id = ?');
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO project_tickets_comments (msg,ticket_id,acc_id,created,last_comment) VALUES (?,?,?,?,?)');
...
?>
<?=template_admin_header($page . ' Project Comment', 'ticketing', 'project')?>
...
<div class="content-title mb-3">
    <div class="title">
        ...
        <div class="txt">
            <h2 class="responsive-width-100"><?=$page?> Project Comment</h2>
            <p>Manage project ticket comments</p>
        </div>
    </div>
</div>
    ...
    <div class="form-professional">
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <textarea id="msg" name="msg" rows="6" placeholder="Enter comment message..." required><?=htmlspecialchars($comment['msg'], ENT_QUOTES)?></textarea>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label for="ticket_id">Project Ticket</label>
        ...
        <div class="form-actions">
            ...
            <?php endif; ?>
            <input type="submit" name="submit" value="<?=$page == 'Edit' ? 'Update' : 'Create'?> Comment" class="btn btn-success">
        </div>
