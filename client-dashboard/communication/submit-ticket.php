<?php
/*******************************************************************************
TICKETING SYSTEM - communication.php
LOCATION: /public_html/client-dashboard/communication/
DESCRIBE: Logged in members can create a help ticket.)
   INPUT: SESSION
LOGGEDIN: REQUIRED
  OUTPUT: 
REQUIRED:
...
DATABASE: TABLES: categories, tickets, tickets_uploads
...
// Include header-process.php BEFORE any HTML output to handle POST redirects
include process_path . 'header-process.php';
//include process_path . 'communication-process.php';
...
// Check if POST data exists (user submitted the TICKETS form)
if (isset($_POST['title'], $_POST['ticket-message'], $_POST['priority'], $_POST['category'])) {
    ...
    if (empty($_POST['title']) || empty($_POST['ticket-message'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO tickets (title, email, msg, priority, category_id, private, acc_id, created, last_update, approved, full_name, last_comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([ $_POST['title'], $email, $_POST['ticket-message'], $_POST['priority'], $_POST['category'], $private, $account_id, date('Y-m-d H:i:s'), date('Y-m-d H:i:s'), $approved, $name,$role ]);
        ...
        // Send the ticket email to the user
           send_ticket_email($email, $ticket_id, $_POST['title'], $_POST['ticket-message'], $_POST['priority'], $category_name, $private, 'open', 'create');

        // Send the ticket email notification to Admin
            $admin_email = notify_admin_email;
            send_ticket_email($admin_email, $ticket_id, $_POST['title'], $_POST['ticket-message'], $_POST['priority'], $category_name, $private, 'open', 'notification', $name, $email);
...
<section class="section profile">
       ...
       <div class="tab-content pt-2">
        <div class="tab-pane fade show active contact-us" id="contact-us">
           <h6 class="card-title"><p class="name"><?=htmlspecialchars($account['full_name'], ENT_QUOTES)?>
            <span class='fs-7' style='float:right'> <?=date('m/d/y h:ia')?></span>
            <br><?=htmlspecialchars($account['email'], ENT_QUOTES)?></h6>
          <div class="row">
            <div class='col-2'></div>
            <div class="col-lg-8">
              <div class="card">
 ...
 <!-- Create a Ticket Form -->
                    ...
                    <div class="col-md-8 col-lg-9">
                        <input aria-labelledby="title" type="text" name="title" class="form-control" placeholder="Title of Ticket" autocomplete="on" id="title" maxlength="<?=max_title_length?>" aria-required="true" required>
                    </div>
                    </div>
