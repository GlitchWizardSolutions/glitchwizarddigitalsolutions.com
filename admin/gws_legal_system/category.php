...
if (isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM gws_legal_categories WHERE id = ?');
    ...
    if (isset($_POST['submit'])) {
        // Update the category
        $stmt = $pdo->prepare('UPDATE gws_legal_categories SET title = ? WHERE id = ?');
    ...
    if (isset($_POST['delete'])) {
        // Delete the category
        $stmt = $pdo->prepare('DELETE FROM gws_legal_categories WHERE id = ?');
        $stmt->execute([ $_GET['id'] ]);
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO gws_legal_categories (title) VALUES (?)');
