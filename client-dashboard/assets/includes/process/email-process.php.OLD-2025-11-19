...
LOG NOTE: 2024-12-27 PRODUCTION  
          ...
          2025-11-18 BUG FIX:   Load PHPMailer once at top to prevent "class already declared" errors.
*/

// Load PHPMailer library ONCE at the top
if (!class_exists('PHPMailer\\PHPMailer\\PHPMailer')) {
    require_once public_path . 'lib/phpmailer/Exception.php';
    require_once public_path . 'lib/phpmailer/PHPMailer.php';
    require_once public_path . 'lib/phpmailer/SMTP.php';
}
...
if (!function_exists('send_ticket_email')){
    ...
    // Log to dedicated debug file for easy access
    $debug_log = public_path . 'debug-email.log';
    file_put_contents($debug_log, date('Y-m-d H:i:s') . " === SEND_TICKET_EMAIL CALLED ===\n", FILE_APPEND);
    file_put_contents($debug_log, "Email: $email, ID: $id, Type: $type\n", FILE_APPEND);
    file_put_contents($debug_log, "SMTP: " . smtp_host . ":" . smtp_port . " User: " . smtp_user . "\n", FILE_APPEND);
    ...
    try {
        ...
        file_put_contents(public_path . 'debug-email.log', date('Y-m-d H:i:s') . " " . $success_msg . "\n", FILE_APPEND);
    } catch (Exception $e) {
        // Output error message
        $error_msg = "TICKET EMAIL ERROR: " . $mail->ErrorInfo . " | EXCEPTION: " . $e->getMessage();
        file_put_contents(public_path . 'debug-email.log', date('Y-m-d H:i:s') . " " . $error_msg . "\n", FILE_APPEND);
...
error_log("Send Mail Parms: email: $email, code: $code, username: $username, type: $type");
error_log("SMTP Config: host=" . smtp_host . ", port=" . smtp_port . ", user=" . smtp_user . ", secure=" . smtp_secure);
...
function send_client_invoice_email($invoice, $client, $subject = '') {
    error_log("send_client_invoice_email");
 ...
 // Include PHPMailer library
	...
	try {
		...
		$email_template = str_replace(
            ...
            [$invoice['invoice_number'], $client['first_name'], number_format($invoice['payment_amount']+$invoice['tax_total'], 2), date('m/d/y', strtotime($invoice['due_date'])), $outside_url . 'client-invoices/invoice.php?id=' . $invoice['invoice_number']],
		...
		// Output error message
		error_log("Error: Invoice Client Message could not be sent. Mailer Error: " . $mail->ErrorInfo);
...
function send_admin_invoice_email($invoice, $client) {
 error_log("send_admin_invoice_email");
