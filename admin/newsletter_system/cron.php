...
Ensure you change the cron_secret constant in the config file as this will prevent direct access to the file!
*/
set_time_limit(0);
...
if (isset($_GET['cron_secret']) && $_GET['cron_secret'] == cron_secret) {
    ...
    $stmt = $pdo->prepare('SELECT ci.*, c.id AS campaign_id, s.email, s.status AS subscriber_status, n.title, n.content, n.attachments, n.id AS newsletter_id FROM campaign_items ci JOIN campaigns c ON c.id = ci.campaign_id AND c.status = "Active" AND c.submit_date < :submit_date JOIN newsletters n ON n.id = c.newsletter_id JOIN subscribers s ON s.id = ci.subscriber_id WHERE ci.status = "Queued" ORDER BY ci.id ASC LIMIT :num_items');
    ...
    // Update campaign status to "Completed" if there are no more items in queue
    $campaigns = $pdo->query('SELECT 
        c.*, 
        (SELECT COUNT(*) FROM campaign_items ci WHERE ci.campaign_id = c.id AND ci.status = "Queued") AS num_queued,  
        (SELECT COUNT(*) FROM campaign_items ci WHERE ci.campaign_id = c.id) AS num_items  
        FROM campaigns c WHERE c.status = "Active"')->fetchAll(PDO::FETCH_ASSOC);
    foreach($campaigns as $campaign) {
        if ($campaign['num_queued'] == 0 && $campaign['num_items'] > 0) {
