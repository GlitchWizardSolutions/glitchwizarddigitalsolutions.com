...
if (isset($_GET['id'])) {
    ...
    $stmt = $logon_db->prepare('SELECT * FROM invoice_clients WHERE id = ?');
...
$stmt = $pdo->prepare('SELECT * FROM domains WHERE invoice_client_id = ? ORDER BY domain');
$stmt->execute([ $_GET['id'] ]);
$domains = $stmt->fetchALL(PDO::FETCH_ASSOC);

$stmt = $pdo->prepare('SELECT c.*, (SELECT COUNT(*) FROM invoices i WHERE i.client_id = c.id) AS total_invoices FROM invoice_clients c WHERE c.id = ? ORDER BY total_invoices');
$stmt->execute([ $_GET['id'] ]);
$clients = $stmt->fetchALL(PDO::FETCH_ASSOC);

// Fetch $projects details associated with the logged-in user
$stmt = $pdo->prepare('SELECT * FROM client_projects WHERE invoice_client_id = ? ORDER BY date_updated');
$stmt->execute([ $_GET['id'] ]);
...
<div class="content-block"  style="background:#7F50AB">
    <div class="table"> 
        <table>
            <thead>
                <tr>
                    <td colspan=3 style='text-align:center; color:white; text-transform: uppercase'><strong><?=htmlspecialchars($record['business_name'], ENT_QUOTES)?></strong></td>
                </tr>
                <tr style='text-align:center'>
                <td style='color:white'></td>
                <td style='color:white'>Invoice: &nbsp;<?=htmlspecialchars($record['first_name'], ENT_QUOTES)?>&nbsp;<?=htmlspecialchars($record['last_name'], ENT_QUOTES)?></td>
                <td style='color:white'></td>
                </tr>
