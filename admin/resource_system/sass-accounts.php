...
if (isset($_GET['delete'])) {
    $stmt = $onthego_db->prepare('DELETE FROM sass_account WHERE id = ?');
    $stmt->execute([ $_GET['delete'] ]);
    header('Location: sass-accounts.php?success_msg=3');
    exit;
}
...
$order_by_whitelist = ['id','resource','details','url','userid','password','email','usage','investment','type','name'];
...
// SQL where clause
$where = '';
$where .= $search ? 'WHERE (resource LIKE :search OR details LIKE :search OR url LIKE :search OR email LIKE :search OR type LIKE :search OR name LIKE :search) ' : '';
// Retrieve the total number of records from the database
$stmt = $onthego_db->prepare('SELECT COUNT(*) AS total FROM sass_account ' . $where);
if ($search) $stmt->bindParam('search', $param3, PDO::PARAM_STR);
$stmt->execute();
$records_total = $stmt->fetchColumn();
// SQL query to get all records from the "sass-account" table
$stmt = $onthego_db->prepare('SELECT * FROM sass_account  ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT :start_results,:num_results');
// Bind params
...
<div class="content-block">
    <div class="table">
        <table>
            <thead>
                <tr>
                    <td ><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=resource'?>">Resource<?php if ($order_by=='resource'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
                    ...
                    <td><?=htmlspecialchars($record['url'], ENT_QUOTES)?></td>
                        ...
                        <div class="table-dropdown">
                            ...
                            <div class="table-dropdown-items">
                                <a href="sass-account-use.php?id=<?=$record['id']?>" style="color:green">
                                    <span class="icon">
                                        <i class="fa-regular fa-credit-card fa-xs"></i>
                                    </span>
                                    Use
                                </a>
                                 <a href="sass-account-view.php?id=<?=$record['id']?>" style="color:blue">
                                    <span class="icon">
                                      <i class="fa-regular fa-eye fa-xs"></i>
                                    </span>
                                    View
                                </a>
                                <a href="sass-account.php?id=<?=$record['id']?>" style='color:orange'>
                                    <span class="icon">
                                     <i class="fa-regular fa-pen-to-square fa-xs"></i>
                                    </span>
                                    Edit
                                </a>
                                <a class="red" href="sass-accounts.php?delete=<?=$record['id']?>" onclick="return confirm('Are you sure you want to delete this account?')">
                                    <span class="icon">
