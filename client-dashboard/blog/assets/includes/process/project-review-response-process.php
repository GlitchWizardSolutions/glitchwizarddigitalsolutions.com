...
$stmt = $pdo->prepare('SELECT * FROM project_tickets WHERE acc_id = ? AND approved = 1 AND ticket_status != "closed" AND last_comment = "Admin" ORDER BY created DESC');
$stmt->execute([ $_SESSION['id'] ]);
$action_required_tickets = $stmt->fetchAll(PDO::FETCH_ASSOC);

/*
// Retrieves the display information for all the project_tickets where admin has replied.
$stmt = $pdo->prepare('SELECT t.*, a.full_name AS a_name, a.email AS a_email, c.title AS category FROM project_tickets t LEFT JOIN project_categories c ON c.id = t.category_id LEFT JOIN accounts a ON a.id = t.acc_id WHERE a.id = ?');
$stmt->execute([ $_SESSION['id']]);
$all_req_ticket = $stmt->fetchAll(PDO::FETCH_ASSOC);*/
...
$stmt = $pdo->prepare('SELECT * FROM project_tickets WHERE acc_id = ? AND approved = 1 AND ticket_status = "paused" ORDER BY created DESC');
$stmt->execute([ $_SESSION['id'] ]);
