...
// Retrieve the total number of open tickets that are not project tickets
...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM project_tickets WHERE ticket_status != "closed" AND ticket_status != "paused" AND priority = "critical" ORDER BY created DESC');
$stmt->execute();
...
// Retrieve 3 high priority projects
$stmt = $pdo->prepare('SELECT * FROM project_tickets WHERE ticket_status != "closed" AND ticket_status != "paused" AND priority = "critical" ORDER BY created DESC LIMIT 3');
$stmt->execute();
$admin_project_critical = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Retrieve 3 high priority projects
$stmt = $pdo->prepare('SELECT * FROM project_tickets WHERE ticket_status != "closed" AND ticket_status != "paused" AND priority = "high" ORDER BY created DESC LIMIT 3');
$stmt->execute();
...
if (isset($_POST['respond_project_id'])) {
           ...
           //redirect the user to the view-ticket page.
				header('Location: https://glitchwizarddigitalsolutions.com/client-dashboard/communication/project-ticket-view.php');
				exit;
			}		
// Handle respond to ticket post data
...
if (isset($_POST['user_document_form'])) {
           ...
           //redirect the user to the view-ticket page.
				header('Location: https://glitchwizarddigitalsolutions.com/client-documents/system-client-data.php');
				exit;
			}			
