...
$stmt = $pdo->prepare('SELECT COUNT(*) AS total, tc.* FROM tickets_comments tc JOIN tickets t ON t.id = tc.ticket_id LEFT JOIN accounts a ON a.id = tc.acc_id ' . $where . ' GROUP BY tc.id ');
...
?>
<?=template_admin_header('Ticket Comments', 'ticketing', 'client')?>
