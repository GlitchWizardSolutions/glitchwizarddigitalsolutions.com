<?php
/*******************************************************************************
CLIENT TICKETING SYSTEM - tickets.php
LOCATION: /public_html/admin/ticketing_system
...
//Open Tickets Data
$stmt = $pdo->prepare('SELECT t.*, (SELECT count(*) FROM tickets_comments tc WHERE t.id = tc.ticket_id) AS msgs, c.title AS category FROM tickets t LEFT JOIN categories c ON c.id = t.category_id WHERE t.ticket_status = "open" ORDER BY t.priority DESC, t.last_comment ASC');
$stmt->execute();
...
<div class="content-block">
    <div class="table">
        <table>
            <thead>
                <tr>
                    <td colspan="2" class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=full_name'?>">Member <?=$order_by=='full_name' ? $table_icons[strtolower($order)] : ''?></a></td>
                    <td></td>
                    <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=title'?>">&nbsp;&nbsp;&nbsp;Ticket Subject<?=$order_by=='title' ? $table_icons[strtolower($order)] : ''?></a></td>
                    ...
                    <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=last_comment'?>">Reply&nbsp;<?=$order_by=='last_comment' ? $table_icons[strtolower($order)] : ''?></a></td>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td colspan="2" class="alt responsive-hidden user">
                        <?=htmlspecialchars($ticket['p_full_name'] ?? $ticket['full_name'], ENT_QUOTES)?>
                        <span><?php /*<?$ticket['p_email'] ?? $ticket['email']?>*/?></span>
                    </td>&nbsp;&nbsp;&nbsp;
                    <td ><?=htmlspecialchars($ticket['title'], ENT_QUOTES)?></td>
                    <td><span class="<?=$ticket['ticket_status']=='resolved'?'blue':($ticket['ticket_status']=='closed'?'grey':'orange')?>"><?=ucwords($ticket['ticket_status'])?></span></td>
                    <td class="responsive-hidden"><?=$ticket['last_comment'] ? '<span class="mark yes">&nbsp;&nbsp;&#10003;</span>' : '<span class="mark no">&nbsp;&#45;</span>'?></td>
                    <td class="responsive-hidden"><span class="<?=$ticket['priority']=='low'?'blue':($ticket['priority']=='high'?'red':'orange')?>"><?=ucwords($ticket['priority'])?></span></td>
