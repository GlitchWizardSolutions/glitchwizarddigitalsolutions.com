...
$filename = '?';
$application= 'Resource System - Cache of Information';
$inputs     = '';
$outputs    = '';
$noted      = 'public_html/admin/resource_system/caches.php';
...
if (isset($_GET['delete'])) {
    $stmt = $onthego_db->prepare('DELETE FROM cache WHERE id = ?');
    $stmt->execute([ $_GET['delete'] ]);
    header('Location: caches.php?success_msg=3');
    exit;
}
...
$order_by_whitelist = [ 'description', 'url', 'username', 'password', 'type', 'notes', 'updated'];
$order_by = isset($_GET['order_by']) && in_array($_GET['order_by'], $order_by_whitelist) ? $_GET['order_by'] : 'description';
...
$where .= $search ? 'WHERE (description LIKE :search OR url LIKE :search OR type LIKE :search OR notes LIKE :search) ' : '';
// Retrieve the total number of records from the database
$stmt = $onthego_db->prepare('SELECT COUNT(*) AS total FROM cache ' . $where);
if ($search) $stmt->bindParam('search', $param3, PDO::PARAM_STR);
$stmt->execute();
$records_total = $stmt->fetchColumn();
// SQL query to get all records from the "client-project" table
$stmt = $onthego_db->prepare('SELECT * FROM cache  ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT :start_results,:num_results');
// Bind params
...
// Determine the URL
$url = 'caches.php?search=' . $search . (isset($_GET['page_id']) ? '&page_id=' . $_GET['page_id'] : '');
?>
...
<div class="content-title mb-3">
    ...
    <i class="fa-regular fa-handshake"></i>
        <div class="txt">
            <h2>Cache of Things</h2>
            <p>All Misc Resources</p>
        </div>
    </div>
</div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=type'?>">Type<?php if ($order_by=='type'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
                    <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=url'?>">URL<?php if ($order_by=='url'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
                    ...
                    <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=updated'?>">Updated<?php if ($order_by=='updated'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
                    <td>Actions</td>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td ><?=htmlspecialchars($record['url'], ENT_QUOTES)?></td>
                    <td class="responsive-hidden"><?=htmlspecialchars($record['notes'], ENT_QUOTES)?></td>
                    <td style="text-align:center" class="responsive-hidden"><?=time_elapsed_string($record['updated'])?></td>
                     <td class="actions">
                        <div class="table-dropdown">
                            ...
                            <div class="table-dropdown-items">
                                 ...
                                 <a href="cache-view.php?id=<?=$record['id']?>" style="color:blue">
                                    <span class="icon">
                                ...
                                <a class="red" href="caches.php?delete=<?=$record['id']?>" onclick="return confirm('Are you sure you want to delete this project and ALL logs?')">
                                    <span class="icon">
