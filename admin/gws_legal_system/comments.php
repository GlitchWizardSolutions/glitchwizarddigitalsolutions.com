...
if (isset($_GET['delete'])) {
    $stmt = $pdo->prepare('DELETE FROM gws_legal_comments WHERE id = ?');
...
$stmt = $pdo->prepare('SELECT COUNT(*) AS total, tc.* FROM gws_legal_comments tc JOIN gws_legal t ON t.id = tc.ticket_id LEFT JOIN accounts a ON a.id = tc.acc_id ' . $where . ' GROUP BY tc.id ');
...
<?=template_admin_header('Legal Filing Comments', 'ticketing', 'legal')?>
...
<div class="content-title">
    <div class="title">
        ...
        <div class="txt">
            <h2>Legal Filing Discussion Notes</h2>
            <p>View, manage, and search discussion notes</p>
        </div>
    </div>
</div>
...
<div class="content-header responsive-flex-column pad-top-5">
    <div class="btns">
        <a href="comment.php" class="btn btn-primary">+ Create Note</a>
    </div>
    <form action="" method="get">
        <div class="search">
            <label for="search">
                <input id="search" type="text" name="search" placeholder="Search notes..." value="<?=htmlspecialchars($search, ENT_QUOTES)?>" class="responsive-width-100">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <td  class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=msg'?>">New Comment<?php if ($order_by=='msg'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
                   
                    <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=created'?>">Date<?php if ($order_by=='created'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="20" style="text-align:center;">There are no notes.</td>
                </tr>
                <?php else: ?>
                ...
                <tr>
                   <td><?=$comment['ticket_title']?></td>
                    ...
                    <td  class="responsive-hidden" style="max-width:200px"><?=nl2br(htmlspecialchars($comment['msg'], ENT_QUOTES))?></td>
                     
                    <td><?=date('m/d/y', strtotime($comment['created']))?></td>
