...
$recipient_name = isset($_POST['recipient_name']) ? $_POST['recipient_name'] : 'Dio Moore';

if (empty($recipient_email)) {
    $_SESSION['error_message'] = 'Email address is required';
    header('Location: meds-dio.php');
    exit();
}
...
$temp_file = sys_get_temp_dir() . '/medication_list_dio_' . time() . '.pdf';
$pdf->Output('F', $temp_file);

// Send email with attachment
$subject = 'Medication List - Dio Moore';
$message = "Hello " . htmlspecialchars($recipient_name) . ",\n\n";
$message .= "Please find attached the medication list for Dio Moore as of " . date('F d, Y') . ".\n\n";
...
$result = send_email_with_attachment(
    ...
    $message,
    $temp_file,
    'Medication_List_Dio_Moore_' . date('Y-m-d') . '.pdf'
);
