<?php     
/***********************************************************************************************
GWS LEGAL SYSTEM - legal-view.php -- does not work yet
LOCATION:   /public_html/
DESCRIBE:   
INPUTREQ:   When the user clicks the project, the project id is passed into a session variable.
LOGGEDIN:   REQUIRED
...
LOG NOTE:    
***********************************************************************************************/
echo 'This is not a production page.  Please check code.';
...
if (isset($_POST['new-comment']) && isset($_SESSION['loggedin'])) {
    ...
    // Insert the new comment into the "projects_comments" table
    //project_id of the specific project, the new comment, the id of the person logged in, and the role of the person leaving the comment.
    ...
    $stmt->execute([ $last_comment, $_POST['status'], date('Y-m-d\TH:i:s'), $_SESSION['ticket_id'] ]);
    
if ($_SESSION['role'] =="Admin") { 
   // Send updated ticket email to user
    send_ticket_email($ticket_data['email'], $ticket_data['id'], $ticket_data['title'], $ticket_data['msg'], $ticket_data['priority'], $ticket_data['category'], $ticket_data['private'], $_POST['status'], 'comment');
}else{
    
    $admin_email = notify_admin_email;
          send_ticket_email($admin_email, $ticket_data['id'], $ticket_data['title'], $ticket_data['msg'], $ticket_data['priority'], $ticket_data['category'], $ticket_data['private'], $_POST['status'], 'notification-comment', $name, $email);
}
    //Redirect to egal-view page
    header('Location: legal-view.php');
    exit;
}
}
  ...
  <div class="col-xl-12 col-lg-12 col-sm-12">
    <div class="card">
     <div class="card-body pt-3">
         <?php if ($ticket_data['last_comment'] == '0' and $ticket_data['ticket_status'] == 'open'): ?>
       ...
       <?php else : ?>
                 <h6 class="card-title">#<?=$ticket_data['id'] ?> 
                    <?php if ($ticket_data['last_comment'] == '0' and $ticket_data['ticket_status'] == 'open'): ?>
                     ...
                     <?php elseif ($ticket_data['last_comment'] == '1' and $ticket_data['ticket_status'] == 'open'): ?> 
...
<!--leftside-->
 ...
 <div class="block tickets view"> 
    ...
    <div class="profile">
        ...
        <div class="info">
            <p class="name"><?=htmlspecialchars($ticket_data['a_name'] ?? $ticket_data['full_name'], ENT_QUOTES)?></p>
        </div><!--info-->
    </div><!--profile-->
    <div class="info">
...
<div class="comments mt-4">
        ...
        <p><span class="comment-header">
                    ...
                    <div style='float:left'>
                      <div><i class="fas fa-comment fa-2x" aria-hidden="true" style="color:#766590;"></i>
                      </div>
                    </div>
                   <?php endif; ?>
                   ...
                   <span class="name<?=$comment['role'] == 'Admin' ? ' is-admin' : ''?>"><?=htmlspecialchars($comment['full_name'] ?? '', ENT_QUOTES)?></span>  
                    ...
                    <?php endif; ?>
                    <span class="date"><?=date('m/d/y h:ia', strtotime($comment['created']))?>  <?php if ($comment['role'] == 'Member'): ?>
