...
if (!isset($_SESSION['ticket_id'])){
     //Redirect  
    header('Location: gws-legal-review-responses.php');
    exit;
}
...
// MySQL query that selects the record by the session ticket id
$stmt = $pdo->prepare('SELECT t.*, a.full_name AS a_name, a.email AS a_email, c.title AS category FROM gws_legal t LEFT JOIN gws_legal_categories c ON c.id = t.category_id LEFT JOIN accounts a ON a.id = t.acc_id WHERE t.id = ?');
...
// Retrieve all of the gws_legal_ that are due to be reviewed this month.
$stmt = $pdo->prepare('SELECT * FROM gws_legal WHERE ticket_status != "closed" AND reminder_date < date_sub(now(), interval 1 month) ORDER BY reminder_date DESC');
$stmt->execute();
...
if (isset($_POST['title'], $_POST['ticket-message'], $_POST['priority'], $_POST['category']) && (isset($_SESSION['loggedin']))) {
    ...
    if (empty($_POST['title']) || empty($_POST['ticket-message']) || empty($_POST['priority'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO gws_legal (title, email, msg, priority, category_id, private, acc_id, created, approved, full_name, reminder_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
