...
if (isset($_GET['delete'])) {
    $stmt = $login_db->prepare('DELETE FROM project_types WHERE id = ?');
    $stmt->execute([ $_GET['delete'] ]);
    header('Location: project-typess.php?success_msg=3');
    exit;
}
...
$order_by_whitelist = ['id', 'name', 'description', 'amount', 'frequency'];
$order_by = isset($_GET['order_by']) && in_array($_GET['order_by'], $order_by_whitelist) ? $_GET['order_by'] : 'name';
...
// SQL where clause
$where = '';
$where .= $search ? 'WHERE (name LIKE :search OR description LIKE :search OR amount LIKE :search OR frequency LIKE :search) ' : '';
// Retrieve the total number of records from the database
$stmt = $login_db->prepare('SELECT COUNT(*) AS total FROM project_types ' . $where);
if ($search) $stmt->bindParam('search', $param3, PDO::PARAM_STR);
$stmt->execute();
$records_total = $stmt->fetchColumn();
// SQL query to get all records from the access resource table
$stmt = $login_db->prepare('SELECT * FROM project_types  ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT :start_results,:num_results');
// Bind params
...
// Determine the URL
$url = 'project-types.php?search=' . $search . (isset($_GET['page_id']) ? '&page_id=' . $_GET['page_id'] : '');
?>
        ...
        <table>
            <thead> 
                <tr>
                    ...
                    <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=name'?>">Project<?php if ($order_by=='name'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
                   
                         <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=id'?>">Deliverables<?php if ($order_by=='deliverables'): ?><i class="fas fa-level-<?=str_replace(['ASC', 'DESC'], ['up','down'], $order)?>-alt fa-xs"></i><?php endif; ?></a></td>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?=htmlspecialchars($record['deliverables'], ENT_QUOTES)?></td>
                     ...
                     <td class="actions" style="text-align: center;">
                        <div class="table-dropdown">
                            ...
                            <div class="table-dropdown-items">
                                 <a href="project-type-view.php?id=<?=$record['id']?>" style="color:blue">
                                    <span class="icon">
                                ...
                                <a class="red" href="project-types.php?delete=<?=$record['id']?>" onclick="return confirm('Are you sure you want to delete this account?')">
                                    <span class="icon">
