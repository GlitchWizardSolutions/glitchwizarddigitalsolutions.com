...
$stmt = $pdo->prepare('SELECT tc.*, a.full_name, a.role FROM tickets_comments tc LEFT JOIN accounts a ON a.id = tc.acc_id WHERE tc.ticket_id = ? ORDER BY tc.created');
$stmt->execute([ $_GET['id'] ]);
...
if (isset($_GET['status'], $_SESSION['loggedin']) && ($_SESSION['role'] == 'Admin' || $_SESSION['id'] == $ticket['acc_id']) && in_array($_GET['status'], ['closed', 'resolved']) && $ticket['ticket_status'] == 'open') {
    ...
    send_ticket_email($ticket['email'], $ticket['id'], $ticket['title'], $ticket['msg'], $ticket['priority'], $ticket['category'], $ticket['private'], $_GET['status'], 'update', '', '', $ticket['acc_id']);
...
if (isset($_POST['msg'], $_SESSION['loggedin']) && !empty($_POST['msg']) && $ticket['ticket_status'] == 'open') {
    ...
    $stmt = $pdo->prepare('INSERT INTO tickets_comments (ticket_id, msg, id) VALUES (?, ?, ?)');
    $stmt->execute([ $_GET['id'], $_POST['msg'], $_SESSION['id'] ]);
    // Send updated ticket email to user
    send_ticket_email($ticket['email'], $ticket['id'], $ticket['title'], $ticket['msg'], $ticket['priority'], $ticket['category'], $ticket['private'], $ticket['status'], 'comment', '', '', $ticket['acc_id']);
    // Redirect to ticket page
    header('Location: admin-ticket-view.php?id=' . $_GET['id']);
    exit;
}
...
?>   
<?=template_admin_header('View Ticket', 'ticketing', 'client')?>
