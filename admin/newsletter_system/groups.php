...
$order_by_whitelist = ['id','title','description','submit_date','subscribers'];
...
if (isset($_GET['success_msg'])) {
    ...
    if ($_GET['success_msg'] == 4) {
        $success_msg = 'Group(s) imported successfully! ' . $_GET['imported'] . ' group(s) were imported.';
    }
}
// Determine the URL
    ...
    <form action="groups.php" method="get">
        ...
        <div class="search">
            <label for="search_query">
                <input id="search_query" type="text" name="search_query" placeholder="Search group..." value="<?=htmlspecialchars($search, ENT_QUOTES)?>" class="responsive-width-100">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=description'?>">Description<?=$order_by=='description' ? $table_icons[strtolower($order)] : ''?></a></td>
                    <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=subscribers'?>"># Subscribers<?=$order_by=='subscribers' ? $table_icons[strtolower($order)] : ''?></a></td>
            ...
            <tbody>
                <?php if (empty($groups)): ?>
                <tr>
                    <td colspan="20" class="no-results">There are no groups.</td>
                </tr>
                <?php else: ?>
                <?php foreach ($groups as $group): ?>
                <tr>
                    <td class="responsive-hidden alt"><?=$group['id']?></td>
                    <td><?=htmlspecialchars($group['title'], ENT_QUOTES)?></td>
                    <td class="responsive-hidden alt"><?=htmlspecialchars($group['description'], ENT_QUOTES)?></td>
                    <td class="responsive-hidden"><a href="subscribers.php?group=<?=$group['id']?>" class="link1"><?=$group['subscribers']?></a></td>
                    <td class="alt"><?=date('F j, Y H:ia', strtotime($group['submit_date']))?></td>
                    <td class="actions">
                        <div class="table-dropdown">
                            ...
                            <div class="table-dropdown-items">
                                ...
                                <a class="red" href="groups.php?delete=<?=$group['id']?>" onclick="return confirm('Are you sure you want to delete this group?')">
                                    <span class="icon">
...
<div class="pagination">
    ...
    <?php endif; ?>
    <span>Page <?=$pagination_page?> of <?=ceil($groups_total / $results_per_page) == 0 ? 1 : ceil($groups_total / $results_per_page)?></span>
    <?php if ($pagination_page * $results_per_page < $groups_total): ?>
