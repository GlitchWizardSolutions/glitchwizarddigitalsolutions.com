...
if (isset($_POST['submit'])){
    ...
    if (!$error_msg) {
        // Update the record
            $stmt = $budget_pdo->prepare('UPDATE budgets 
            SET
            ...
            $_POST['reference'],
            $_GET['id'] ]);
        // Get the updated budget from the budgets table
        $stmt = $budget_pdo->prepare('SELECT * FROM budgets WHERE id = ?');
        $stmt->execute([ $_GET['id'] ]);
        $budget_id= $stmt->fetch(PDO::FETCH_ASSOC);
          // Output message
...
<td>   
            ...
            <option value='<?=$budget_id['hancock'] ?>' selected><?=$budget_id['hancock']?></option>
...
<td>
         	<div class="form-control" style='width:50%'>
                ...
                <input type="number" name="amount"  id="amount" value="<?=$budget_id['amount']?>" required>
             </div>
</td>
</tr>
<tr>
 <td><p style='color:red'>This is updated via web applications.</p>
      	  <div class="form-control" style='width:50%'>
                ...
                <input type="number" name="balance"  id="balance"value="<?=$budget_id['balance']?>" required>
             </div>
</td>
</tr>
<tr>
...
<td>
   ...
   <a href="budgets_browse.php" class="btn alt mar-right-2">Cancel</a>
