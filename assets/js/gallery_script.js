...
const media_toggle_like = media_link => {
	...
	like_btn.onclick = event => {
		...
		fetch('content-like.php?id=' + media_link.dataset.id, { cache: 'no-store' }).then(res => res.text()).then(data => {
		...
		document.body.insertAdjacentHTML('beforebegin', `
			<div class="media-save">
				<div class="con">
					...
					<form action="content-add-media-to-collection.php" method="post" class="gallery-form full">
			...
			if (media_link.dataset.type == 'image') {
				...
				img.onload = () => {
					...
					media_popup.innerHTML = `
						...
						<div class="con">
							<h3>${media_link.dataset.title}${media_link.dataset.ownMedia !== undefined ? '<a class="edit-media-btn" href="content-manage.php?id=' + media_link.dataset.id + '"><i class="fa-solid fa-pen"></i></a>' : ''}</h3>
							<p>${media_meta.alt}</p>
							...
							<div class="like-con">
								...
								<div class="action-btns">
									...
									${media_link.dataset.collection ? '<a href="content-delete-media-from-collection.php?collection_id=' + media_link.dataset.collection + '&media_id=' + media_link.dataset.id + '" onclick="return confirm(\'Remove media from collection?\')"><i class="fa-solid fa-trash"></i></a>' : ''}
								</div>
							</div>
						</div>
		...
		for (let i = 0; i < upload_media.files.length; i++) {
			...
			document.querySelector("#submit_btn").insertAdjacentHTML('beforebegin', `
				...
				<!--	<option value="1">Everyone</option> -->
					<option value="0">Myself and the Webmaster</option>
				</select>
