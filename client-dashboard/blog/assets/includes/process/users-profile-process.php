...
if (isset($_POST['username'], $_POST['email'], $_POST['full_name'])) {
	...
	if (empty($_POST['username']) || empty($_POST['full_name']) || empty($_POST['email'])) {
	...
	if (empty($msg)) {
		...
		if ($result = $stmt->fetchColumn()) {
			...
			$stmt = $pdo->prepare('UPDATE accounts SET username = ?,  email = ?, activation_code = ?, full_name = ?, phone = ?, address_street = ?, address_city = ?, address_state = ?, address_zip = ? WHERE id = ?');
			$stmt->execute([ $_POST['username'], $_POST['email'], $uniqid, $_POST['full_name'], $_POST['phone'], $_POST['address_street'], $_POST['address_city'], $_POST['address_state'], $_POST['address_zip'], $_SESSION['id'] ]);
...
$stmt = $pdo->prepare('SELECT * FROM invoice_clients WHERE acc_id = ?');
$stmt->execute([ $_SESSION['id'] ]);
