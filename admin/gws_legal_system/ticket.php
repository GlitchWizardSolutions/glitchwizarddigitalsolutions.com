...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        // Update the ticket
        $stmt = $pdo->prepare('UPDATE gws_legal SET title = ?, msg = ?, full_name = ?, email = ?, created = ?, ticket_status = ?, priority = ?, category_id = ?, private = ?, acc_id = ?, approved = ? WHERE id = ?');
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO gws_legal (title, msg, full_name, email, created, ticket_status, priority, category_id, private, acc_id, approved) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
...
?>
<?=template_admin_header($page . ' GWS Legal Requirement', 'legal', 'legal')?>
