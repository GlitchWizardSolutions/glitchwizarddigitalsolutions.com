<?php
$ticket_view_url= site_menu_base . 'client-dashboard/gws-legal/gws-legal-view.php';
...
// Need to have this by if it's almost due or not, later.
$stmt = $pdo->prepare('SELECT * FROM gws_legal WHERE ticket_status != "closed" ORDER BY created DESC');
$stmt->execute();
$action_required_tickets = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Retrieve the open gws_legal from the database
$stmt = $pdo->prepare('SELECT * FROM gws_legal WHERE ticket_status = "open" ORDER BY created DESC');
$stmt->execute();
$open_tickets = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Retrieve the paused gws_legal from the database
$stmt = $pdo->prepare('SELECT * FROM gws_legal WHERE ticket_status = "paused" ORDER BY created DESC');
$stmt->execute();
$paused_tickets = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Retrieve the closed gws_legal from the database
$stmt = $pdo->prepare('SELECT * FROM gws_legal WHERE ticket_status = "closed" ORDER BY created DESC');
$stmt->execute();
...
// Update ticket status in the database
    $stmt = $pdo->prepare('UPDATE gws_legal SET last_comment = ?, ticket_status = ? WHERE id = ?');
...
if (isset($_POST['msg']) and !empty($_POST['msg']) and $action_required_tickets['ticket_status'] == 'open') {
    ...
    //Redirect 
    header('Location: gwslegal-view.php');
    exit;
}
...
if (isset($_POST['respond_ticket_id2'])) {
           ...
           //redirect the user to the view-ticket page.
				header('Location: https://glitchwizarddigitalsolutions.com/client-dashboard/gws-legal/gws-legal-view.php');
				exit;
			}
...
$stmt = $pdo->prepare('SELECT t.*, a.full_name AS a_name, a.email AS a_email, c.title AS category FROM gws_legal t LEFT JOIN categories c ON c.id = t.category_id LEFT JOIN accounts a ON a.id = t.acc_id WHERE t.id = ?');
...
$stmt = $pdo->prepare('SELECT tc.*, a.full_name, a.role FROM gws_legal_comments tc LEFT JOIN accounts a ON a.id = tc.acc_id WHERE tc.ticket_id = ? ORDER BY tc.created DESC');
$stmt->execute([ $ar_ticket['id'] ]);
