...
if (isset($_POST['save'])) {
    ...
    $stmt = $blog_pdo->prepare("SELECT * FROM `users` WHERE email=? AND id != ? LIMIT 1");
    $stmt->execute([$email, $user_id]);
    $countue = $stmt->rowCount();
    if ($countue > 0) {
        $emused = 'Yes';
    }
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) && $emused == 'No') {
        ...
        if ($password != null) {
            ...
            $stmt = $blog_pdo->prepare("UPDATE `users` SET email=?, username=?, avatar=?, password=? WHERE id=?");
            $stmt->execute([$email, $username, $avatar, $password, $user_id]);
        } else {
            $stmt = $blog_pdo->prepare("UPDATE `users` SET email=?, username=?, avatar=? WHERE id=?");
            $stmt->execute([$email, $username, $avatar, $user_id]);
        }
