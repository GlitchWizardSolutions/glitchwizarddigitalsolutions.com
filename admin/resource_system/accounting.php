...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
                $stmt = $onthego_db->prepare('UPDATE financial_cards SET name = ?, card_number = ?, expires = ?, code = ?, brand = ?, zip_code = ?, card_type = ? WHERE id = ?');
                $stmt->execute([ $_POST['name'], $_POST['card_number'], $_POST['expires'], $_POST['code'], $_POST['brand'], $_POST['zip_code'], $_POST['card_type'], $_GET['id'] ]);
                header('Location: is_importants.php?success_msg=2');
                exit;
    ...
    if (isset($_POST['delete'])) {
        ...
        header('Location: is_importants.php?delete=' . $_GET['id']);
        exit;
    }
    ...
    $page = 'Create';
        if (isset($_POST['submit'])) {
                $stmt = $onthego_db->prepare('INSERT INTO financial_cards (name, card_number, expires, code, brand, zip_code, card_type) VALUES (?, ?, ?, ?, ?, ?, ?)');
                $stmt->execute([ $_POST['name'], $_POST['card_number'], $_POST['expires'],  $_POST['code'], $_POST['brand'], $_POST['zip_code'] , $_POST['card_type']]);
                header('Location: is_importants.php?success_msg=1');
                exit;
            }
}
?>
        ...
        <div class="form responsive-width-100">
            ...
            <label for="brand">Brand</label>
                ...
                <select name="card_type" id="card_type">
                    ...
                    <option value="debit"<?=$record['card_type']=='debit'?' selected':''?>>Debit</option>
                    <option value="gift"<?=$record['card_type']=='gift'?' selected':''?>>Gift</option>
                </select>
        </div>
