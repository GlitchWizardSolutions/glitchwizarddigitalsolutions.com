...
$selected='';
$match='';
$stmt =$budget_pdo->prepare('SELECT * FROM categories');
$stmt->execute();
$categories = $stmt->fetchAll(PDO::FETCH_ASSOC);


$stmt =$budget_pdo->prepare('SELECT * FROM subcategories');
$stmt->execute();
...
// Column list
$columns = ['bill',  'next_due_date', 'next_due_amount', 'description'];
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
	...
	$where_sql .= ($where_sql ? ' AND ' : ' WHERE ') .  'description LIKE :search_query OR bill LIKE :search_query';
}
// Limit SQL
$limit_sql = '';
		...
		<div class="table">
			<table>
				<thead>
					<tr>
 						<td<?=$order_by=='bill'?' class="active"':''?>>
							<a href="bills_browse.php?page=1&records_per_page=<?=$records_per_page?>&order_by=bill&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
							 Bill
							...
							<a href="bills_browse.php?page=1&records_per_page=<?=$records_per_page?>&order_by=next_due_date&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
								Date Due
							...
							<a href="bills_browse.php?page=1&records_per_page=<?=$records_per_page?>&order_by=next_due_amount&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
							   Due
						...
						<td<?=$order_by=='description'?' class="active"':''?>>
							<a href="bills_browse.php?page=1&records_per_page=<?=$records_per_page?>&order_by=description&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
							Description
				...
				<tbody>
					...
					<td class="last_paid_amount right"><?=htmlspecialchars($result['description']?? '', ENT_QUOTES)?></td>
						<td class="actions">
			...
			<div class="pagination">
				...
				<?php if ($page > 1): ?>
				<a href="bills_browse.php?page=<?=$page-1?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>" class="prev">
					<i class="fa-solid fa-angle-left"></i> Prev
				</a>
				<?php endif; ?>
				<?php if ($page > 1): ?>
				<a href="bills_browse.php?page=1&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">1</a>
				<?php endif; ?>
				<?php if ($page > 2): ?>
				<div class="dots">...</div>
				<?php if ($page == ceil($num_results/$records_per_page) && ceil($num_results/$records_per_page) > 3): ?>
				<a href="bills_browse.php?page=<?=$page-2?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>"><?=$page-2?></a>
				<?php endif; ?>
				<?php endif; ?>
				<?php if ($page-1 > 1): ?>
				<a href="bills_browse.php?page=<?=$page-1?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>"><?=$page-1?></a>
				<?php endif; ?>
				<a href="bills_browse.php?page=<?=$page?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>" class="selected"><?=$page?></a>
				...
				<a href="bills_browse.php?page=<?=ceil($num_results/$records_per_page)?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>"><?=ceil($num_results/$records_per_page)?></a>
				<?php endif; ?>
				<?php if ($records_per_page != 'all' && $page < ceil($num_results/$records_per_page)): ?>
				<a href="bills_browse.php?page=<?=$page+1?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?> <?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>" class="next">
