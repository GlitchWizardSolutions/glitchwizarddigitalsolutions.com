...
if (isset($_GET['approve'])) {
    $stmt = $pdo->prepare('UPDATE project_tickets SET approved = 1 WHERE id = ?');
...
$order_by_whitelist = ['ticket_status', 'last_comment', 'created','priority','category_id','title','msg'];
...
$stmt = $pdo->prepare('SELECT COUNT(*) AS total FROM project_tickets t ' . $where);
...
//Open Tickets Data
$stmt = $pdo->prepare('SELECT t.*, (SELECT count(*) FROM project_tickets_comments tc WHERE t.id = tc.ticket_id) AS msgs, c.title AS category FROM project_tickets t LEFT JOIN project_categories c ON c.id = t.category_id WHERE t.ticket_status = "open" ORDER BY t.priority DESC, t.last_comment ASC');
$stmt->execute();
...
$stmt = $pdo->prepare('SELECT COUNT(*) AS total FROM project_tickets WHERE ticket_status = "paused"');
$stmt->execute();
...
<div class="content-title">
    <div class="title">
        ...
        <div class="txt">
                  <h2 class="responsive-width-100">Projects System</h2>
            <p>Manage Projects</p>
        </div>
    </div>
</div>
...
<div class="filter-list"> 
    ...
    <div class="filter">
        ...
        Ticket Status : <?=htmlspecialchars($status, ENT_QUOTES)?>
    </div>
    <?php endif; ?>
        ...
        <table>
            <thead>
                <tr style="text-align:center">
                    <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=id'?>"> Ticket # <?=$order_by=='id' ? $table_icons[strtolower($order)] : ''?></a></td> 
                    ...
                    <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=priority'?>">Priority<?=$order_by=='priority' ? $table_icons[strtolower($order)] : ''?></a></td>
                    ...
                    <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=created'?>">Created<?=$order_by=='created' ? $table_icons[strtolower($order)] : ''?></a></td>
                     <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=reminder_date'?>">Review Due<?=$order_by=='reminder_date' ? $table_icons[strtolower($order)] : ''?></a></td>
                    <td>Actions</td>
                </tr>
            </thead>
            <tbody>
                ...
                if($ticket['priority']=='low'){
                    ...
                    $priority_color = 'red';
                }else if($ticket['priority']=='paused'){
                    $priority_color = 'grey';
                }else if($ticket['priority']=='closed'){
                    $priority_color = 'grey';
                }
                ...
                <tr>
                    <td style="text-align:center"><?=htmlspecialchars($ticket['id'], ENT_QUOTES)?></td> 
                    <td><?=htmlspecialchars($ticket['title'], ENT_QUOTES)?></td>
                    <td style="text-align:center"><span class="<?=$ticket['ticket_status']=='resolved'?'blue':($ticket['ticket_status']=='open'?'green':'grey')?>"><?=ucwords($ticket['ticket_status'])?></span></td>
                    <td class="responsive-hidden" style="text-align:center"><span class="<?=$priority_color  ?>"><?=ucwords($ticket['priority'])?></span></td>
                    <td class="responsive-hidden" style="text-align:center"><span class="<?=$category_color ?>"><?=$ticket['category']?></span></td>
                    <td class="alt responsive-hidden" style="text-align:center"><?=time_elapsed_string($ticket['created'])?></td>
                    <?php if($ticket['reminder_date']=='9999-12-31'):?>
                    <td class="alt responsive-hidden" style="text-align:center">none</td>
                    <?php else:?>
                        <?php if($current_date > $ticket['reminder_date']): ?>
                          <td class="alt responsive-hidden" style="text-align:center"><span style="color:red"> <?=time_difference_string($ticket['reminder_date'])?></span></td>
                          <?php elseif($current_date < $ticket['reminder_date']): ?>
                           <td class="alt responsive-hidden" style="text-align:center"><span style="color:blue"><?=time_difference_string($ticket['reminder_date'])?></span></td>
                           <?php else: ?>  
                           <td class="alt responsive-hidden" style="text-align:center"><span style="color:green"><?=time_difference_string($ticket['reminder_date'])?></span></td>
 <?php endif?>
                    <?php endif?>
                    <td class="actions">
