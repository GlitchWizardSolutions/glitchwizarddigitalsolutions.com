...
$recipient_name = isset($_POST['recipient_name']) ? $_POST['recipient_name'] : 'Joseph Gross';

if (empty($recipient_email)) {
    $_SESSION['error_message'] = 'Email address is required';
    header('Location: meds-joseph.php');
    exit();
}
...
class PDF extends FPDF {
	function Header() {
		...
		$this->Cell(0,10,'Medication List - Joseph Gross',0,1,'C');
...
// Fetch medications
$meds_result = $onthego_db->query('SELECT * FROM meds WHERE patient = "Joseph Gross" and status = "Active" ORDER BY name ASC');
...
$temp_file = sys_get_temp_dir() . '/medication_list_joseph_' . time() . '.pdf';
$pdf->Output('F', $temp_file);

// Send email with attachment
$subject = 'Medication List - Joseph Gross';
$message = "Hello " . htmlspecialchars($recipient_name) . ",\n\n";
$message .= "Please find attached the medication list for Joseph Gross as of " . date('F d, Y') . ".\n\n";
...
$result = send_email_with_attachment(
    ...
    $message,
    $temp_file,
    'Medication_List_Joseph_Moore_' . date('Y-m-d') . '.pdf'
);
