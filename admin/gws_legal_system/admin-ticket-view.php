<?php
/*******************************************************************************
GWS Legal Requirement SYSTEM - tickets.php
...
$stmt = $pdo->prepare('SELECT tc.*, a.full_name, a.role FROM gws_legal_comments tc LEFT JOIN accounts a ON a.id = tc.acc_id WHERE tc.ticket_id = ? ORDER BY tc.created');
$stmt->execute([ $_GET['id'] ]);
...
if (isset($_GET['status'], $_SESSION['loggedin']) && ($_SESSION['role'] == 'Admin' || $_SESSION['id'] == $ticket['acc_id']) && in_array($_GET['status'], ['closed', 'resolved']) && $ticket['ticket_status'] == 'open') {
    ...
    $stmt = $pdo->prepare('UPDATE gws_legal SET ticket_status = ? WHERE id = ?');
...
if (isset($_POST['msg'], $_SESSION['loggedin']) && !empty($_POST['msg']) && $ticket['ticket_status'] == 'open') {
    ...
    $stmt = $pdo->prepare('INSERT INTO gws_legal_comments (ticket_id, msg, id) VALUES (?, ?, ?)');
