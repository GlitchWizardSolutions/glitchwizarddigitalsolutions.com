...
$copy="";
    ...
    $stmt = $logon_db->prepare('SELECT client_projects_id, client_note, dev_note, private_dev_note, date_created FROM client_projects_logs WHERE client_projects_id = ? ORDER BY date_created DESC');
    $stmt->execute([ $_GET['id'] ]);
    $project_logs = $stmt->fetchALL(PDO::FETCH_ASSOC); 
?>
    <?=template_admin_header($page . ' Client Project Log', 'resources', 'logs')?>
